  ...
  <div class="dashboard-container">
    ...
    <el-card class="box-card" style="margin-top: 30px" shadow="never">
      <el-table border v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="类型名称" align="center" prop="name" width="150"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px">
        <el-form-item label="属性名称" prop="icon">
          <el-input v-model="form.name" type="textarea" placeholder="请输入属性名称" />
        </el-form-item>
      </el-form>
...
import { listProductType, getProductType, addProductType, updateProductType, delProductType } from "@/api/product/productType";

const defaultAdminProductType = {
  // 查询参数
  id: null,
  name: null,
  attributeCount: null,
...
export default {
  ...
  data() {
    return {
      adminProductType: Object.assign({}, defaultAdminProductType),
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductType(this.listQuery).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除商品管理商品类型资源编号为"' + ids + '"的数据项?', "警告", {
